...
const unblock = function unblock($port, callback) {
  ...
  (global.hosts = {}),
    hook.target.host.forEach(host => (global.hosts[host] = config.forceHost));
  ...
  Promise.all(
    ...
    .then(result => {
      let extra = Array.from(
        new Set(result.reduce((merged, array) => merged.concat(array), []))
      );
...
const cancelUnblock = function() {
  process.exit(1);
};

module.exports = { unblock, cancelUnblock };
